//! Minimal IPC helpers for the security service.
//!
//! Mirrors the IPC layout used by other services (e.g., VFS) so the
//! kernel/syscall layer can deliver and receive messages.
...
pub extern "C" fn sys_ipc_send(port_id: u64, msg: *const IpcMessage) -> i32 {
    unsafe { syscall_raw(9, port_id, msg as u64, 0, 0, 0) as i32 }
}

/// System call wrapper for IPC receive
#[no_mangle]
pub extern "C" fn sys_ipc_receive(port_id: u64, msg: *mut IpcMessage) -> i32 {
    unsafe { syscall_raw(10, port_id, msg as u64, 0, 0, 0) as i32 }
}

/// Raw syscall (x86_64 only for now)
